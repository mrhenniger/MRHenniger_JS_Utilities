...
class Files {
    ...
    */
    static getContents(refFile: string|Strings, force: boolean = false): Promises {
...
debugger;
    ...
    * @return  Promises  Resolves with the text content.
    */
    static getJson(refFile: string|Strings, force: boolean = false): Promises {
        refFile = typeof refFile === 'string' ? refFile : refFile.str();

        let ret = new Promises();

        let contentsPromise = Files.getContents(refFile, force);
        contentsPromise.then((content: any) => {
